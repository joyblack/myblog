# 简介
Spark是一种快速、通用、可扩展的大数据分析引擎。通过本章节你可以了解到spark的一些基本定义及其用途。
...
2. 运行整个软件栈的代价变小了。不需要运行多独立的软件系统，一个机构只需要运行一套软件系统即可。系统的部署、维护、测试、支持等大大缩减。
...
* **集群管理器** Spark 设计为可以高效地在一个计算节点到数千个计算节点之间伸缩计 算。为了实现这样的要求，同时获得最大灵活性，Spark 支持在各种集群管理器(cluster manager)上运行，包括 Hadoop YARN、Apache Mesos，以及 Spark 自带的一个简易调度器——独立调度器。 
